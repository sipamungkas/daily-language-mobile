...
type NewsV2ScreenProps = NativeStackScreenProps<StackParamList, 'NewsV2Screen'>;

const NewsV2Screen = (props: NewsV2ScreenProps) => {
  ...
  const getNewsList = useCallback(async (page: number, isRefresh = false) => {
    try {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const res: any = await Reading.createNewsData(body);
      if (res.code === 201) {
  ...
  }, [endTime, getNewsList, startTime, url]);

  return (
    <SafeAreaView>
      <Header showBack title="News/Article" />
