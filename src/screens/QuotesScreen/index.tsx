...
import {Quest, Reading} from '@/services/api';
...
type QuotesScreenProps = NativeStackScreenProps<StackParamList, 'QuotesScreen'>;

const QuotesScreen = (props: QuotesScreenProps) => {
  ...
  const getQuoteList = useCallback(async (page: number, isRefresh = false) => {
    try {
      const res: any = await Reading.quotesList(page);
      if (res.code === 200) {
        if (isRefresh) {
  ...
  const handleLoadMore = useCallback(() => {
    if (meta.page < meta.totalPage && !isLoading) {
      getQuoteList(meta.page + 1);
    }
  }, [getQuoteList, isLoading, meta.page, meta.totalPage]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const body: IBodyCreateNS = {
        url,
        startTime: moment(startTime).format('HH:mm'),
        endTime: moment(endTime).format('HH:mm'),
      };
      const res: any = await Reading.createNativeSocmed(body);
      if (res.code === 201) {
  ...
  }, [endTime, getQuoteList, startTime, url]);

  return (
    <SafeAreaView>
      <Header showBack title="Native Socmed" />
