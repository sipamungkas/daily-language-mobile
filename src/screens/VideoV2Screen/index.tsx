...
type VideoV2ScreenProps = NativeStackScreenProps<
  StackParamList,
  'VideoV2Screen'
>;

const VideoV2Screen = (props: VideoV2ScreenProps) => {
  ...
  const getAudioList = useCallback(async (page: number, isRefresh = false) => {
    try {
  ...
  const handleLoadMore = useCallback(() => {
    if (meta.page < meta.totalPage && !isLoading) {
      getAudioList(meta.page + 1);
    }
  }, [getAudioList, isLoading, meta.page, meta.totalPage]);
  ...
  }, [endTime, getAudioList, startTime, url]);

  return (
    <SafeAreaView>
      <Header showBack title="Watching YouTube" />
